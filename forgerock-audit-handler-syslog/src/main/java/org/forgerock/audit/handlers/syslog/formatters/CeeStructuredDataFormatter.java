 ...
 */
public class CeeStructuredDataFormatter implements StructuredDataFormatter {
    ...
    private static final Set<String> IGNORED_FIELDS = unmodifiableSet(
            new HashSet<>(asList("_id", TIMESTAMP)));
    ...
    public String format(JsonValue auditEvent) {
        ...
        JsonValue filteredEvent = auditEvent.copy();

        for (String fieldName : fieldNames) {
            String formattedName = formatParamName(fieldName);
            if (IGNORED_FIELDS.contains(formattedName)) {
                filteredEvent.remove(new JsonPointer(fieldName));
            }
        }

        sd.append(extractValueAsString(filteredEvent, "/"));
